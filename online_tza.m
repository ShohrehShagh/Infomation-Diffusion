function []=online_tza(N,pi,L0,a_mat,b_mat,mu1,mu2,sigma1,sigma2,data_num,Number_of_batches,Number_of_samples,r,p_b,Burn_in,N_thin,pdf_val)
...
for batch=2:Number_of_batches
    ...
    for m=1:Number_of_samples
        ...
        x_star=Draw(prev_x_star,N,BL,p_b,t_ML',L0,t_start_batch,a_mat,b_mat,pi,1);
        if(m==1)
            ...
            x=refine(x,prev_x,pi,L0,N,a_mat,b_mat,term,t_start_batch,pdf_val,m,1);
        end
        if (m>Burn_in)
    ...
    marginal_tcs(isnan(marginal_tcs))=Inf;
    ...
    hats
    save(sprintf('Final_Results/online_tza_%d_%d_%d',data_num,batch,r),'hats','-v7.3')
end
